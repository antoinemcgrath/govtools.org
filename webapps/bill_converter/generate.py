...
def get_digitalocean_keys():
    with open(home + '/govtools.org/webapps/bill_converter/keys.json') as f:
...
def store_new_ip(ip_address):
    with open(home + "/govtools.org/webapps/bill_converter/ip.txt", "w+") as fi:
        fi.write(ip_address)
...
def scp_files(ip_address):
    print("Status: Attempting scp of upload dir")
    try: # scp -r -o 'StrictHostKeyChecking no' conversion_script root@162.243.14.5:~/
        ...
        "root@" + ip_address + ":~/"])
        print("Status: scp of upload dir success")
...
def ssh_to_command(ip_address, inputgit):
    ...
    if inputgit.find("github.com/") > -1:
        ...
        sshProcess.stdin.write("sh ~/" + inputgit.split('/')[4] + "-master/webapps/bill_converter/run.sh " + "\n")
